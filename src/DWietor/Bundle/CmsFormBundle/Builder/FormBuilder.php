...
use DWietor\Bundle\CmsFormBundle\Form\Type\CmsFormType;
...
use DWietor\Bundle\CmsFormBundle\Validator\ConstraintProviderInterface;
...
class FormBuilder implements FormBuilderInterface
{
    ...
    public function getForm(string $alias, array $options = []): FormInterface
    {
        ...
        if (!array_key_exists('action', $options) || $options['action'] === null || $options['action'] === '') {
            $options['action'] = $this->router->generate('d_wietor_cms_frontend_ajax_respond', [
                'uuid' => $cmsForm->uuid()
            ]);
        }
